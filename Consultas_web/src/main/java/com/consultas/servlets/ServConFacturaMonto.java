...
@WebServlet("/ServConFacturaMonto")
public class ServConFacturaMonto extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        out.print("  <title>Consulta de Monto Facturas</title>");
        ...
        out.print("  <h3>Top Facturas de  mayor monto</h3>");

        // Tabla de resultados
        out.print("  <table>");
        out.print("    <thead>");
        out.print("      <tr><th>ID</th><th>Fecha de Emision</th><th>Monto Total</th></tr>");
        out.print("    </thead>");
        out.print("    <tbody>");
        try (
                Connection conn = Coneccion.conectar();
                Statement stmt = conn.createStatement();
                ResultSet rs = stmt.executeQuery("SELECT f.id, f.fecha_emision, ROUND(SUM(fd.cantidad * p.precio)) AS total\n" +
                        "FROM factura f\n" +
